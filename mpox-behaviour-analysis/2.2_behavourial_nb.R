...
fig_path <- case_when(outcome_var == "nb_part_ttl" ~ sprintf("./behavourial-figures/%s/results-checks-ttl", analysis),
                      outcome_var == "nb_part_anal" ~ sprintf("./behavourial-figures/%s/results-checks-anal", analysis))

out_distr_path <- case_when(outcome_var == "nb_part_ttl" ~ sprintf("./behavourial-outputs/%s/fitted-distr-ttl", analysis),
                            outcome_var == "nb_part_anal" ~ sprintf("./behavourial-outputs/%s/fitted-distr-anal", analysis))
out_distr_pref <- case_when(outcome_var == "nb_part_ttl" ~ sprintf("all-%s", sa_grouping),
                            outcome_var == "nb_part_anal" ~ sprintf("anal-%s", sa_grouping))
...
vars_model <- c("period_mpox_centre",
                ...
                "sa_cats_ttl_higher",
                "period_mpox_centre_sa_cats_ttl_higher")

formula_ttl <- as.formula(paste("nb_part_ttl_visit ~ (1|id) + ", paste(vars_model, collapse = "+")))

num_cores <- parallel::detectCores()
t0 <- Sys.time()
set.seed(77)
fit_ttl <- stan_glmer.nb(
  formula_ttl,
  iter = 4000,
  chains = 2,
  data = data_22,
...
trace <- plot(fit_ttl, "trace", pars = c("(Intercept)", vars_model, "reciprocal_dispersion"))
...
cur_model[cur_model$name == "reciprocal_dispersion", 
          c("mean", "conf_lb", "conf_ub")] <- log(cur_model[cur_model$name == "reciprocal_dispersion", 
                                                            c("mean", "conf_lb", "conf_ub")])

# interaction
cur_model_inter <- interaction_fn(cur_model, 
                                  vcov(fit_ttl),
                                  covariate_name = "sa_cats_ttl", 
                                  covariate_level = c("_lower", "_higher"), 
                                  covariate_ref = "_lower")
...
vars_model_full <- c("Mpox outbreak coverage",
                     ...
                     "Sexual Partner Numbers > 7 at the most recent visit before 2022",
                     "Mpox outbreak coverage x Sexual Partner Numbers > 7")

cur_model <- add_column(cur_model,
                        name_full = c("exp(intercept)", vars_model_full, "inverse_overdisp"),
                        .after = "name")
...
inter_tbl <- cur_model_inter %>% 
  dplyr::select(name, mean, se, conf_lb, conf_ub) %>%
  mutate(Mean = paste0(round(mean, 2), " (", round(conf_lb, 2), ", ", round(conf_ub, 2), ")"),
         SE = signif(se, 2)) %>%
  dplyr::select(name, Mean, SE)

# save coefficients 
write.csv(coeff_tbl, sprintf("%s/table_coef_%s.csv", out_distr_path, out_distr_pref))
write.csv(inter_tbl, sprintf("%s/table_inter_%s.csv", out_distr_path, out_distr_pref))
