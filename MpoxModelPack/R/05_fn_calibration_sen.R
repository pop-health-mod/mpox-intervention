#' @title prior_dens_fix_sen
#' @author: Mathieu Maheu-Giroux, Jorge Luis Flores Anato, Fanyu Xiu
#' @description sum of log prior density for a given set of parameter values (not calibrating RR)
#' @param theta value of a vector of parameter in logit scale 
#' @return a value equals to log prior density
#' @rdname prior_dens_fix_sen
#' @importFrom foreach %dopar%
#' @export 
prior_dens_fix_sen <- function(theta) {
  log_prior <- dnorm(theta[4], mean = qlogis(0.87), sd = 1, log = TRUE) + # beta (transmission parameter)
    dnorm(theta[5], mean = qlogis(0.5), sd = 1, log = TRUE) + # omega (mixing)
    dnorm(theta[6], mean = qlogis((5 - 3) / (15 - 3)), sd = 1, log = TRUE)  # duration of infectiousness (median ~ 5 days) (using transformed qlogis to bound within reasonable values)
    return(log_prior)
}

#' @title llk_all_sen
#' @description function to output posterior density of a parameter set using negative binomial likelihood (not calibrating RR)
#' @param theta value of a vector of parameter in logit scale 
...
llk_all_sen <- function(theta, 
  ...
  for(cal_city in c("mtl", "trt", "van")){
    ...
    model_output <- fn_model(city = cal_city,
                             ...
                             omega_city = plogis(theta[5]),
                             RR_H_city = 0.67,
                             RR_L_city = 0.80,
                             gamma1_city = 1 / (3 + plogis(theta[6]) * (15 - 3)),
                             period_city = 150,
                             VACCINATING = 1,
                             TRACING = 1,
                             cpp = cal_cpp)
...
#' @title simul_fun_all_sen
#' @description simulation to obtain credible interval of all parameter sets (CrI) (not calibrating RR)
#' @param cal_type which type of sexual act to be selected (all-type:1 or anal:2), Default: 1
...
simul_fun_all_sen <- function(cal_type,
                      hessian, 
                      thetas, 
                      sim = 1000, 
                      SIR = TRUE, 
  ...
  for(cty in c("mtl", "trt", "van")){
    ...
    for (s in 1:sim) {
      ...
      tmp <- fn_model(city = cty,
                      ...
                      omega_city = plogis(samp[s, 5]),
                      period_city = period_mod_cty,
                      RR_H_city = 0.67,
                      RR_L_city = 0.80,
                      gamma1_city = 1 / (3 + plogis(samp[s, 6]) * (15 - 3)),
                      VACCINATING = 1,
                      TRACING = 1,
                      cpp = cal_cpp)
